...
Plugin URI: http://superslider.daivmowbray.com/superslider/superslider-show/
...
if (!class_exists("ssShow")) {
	...
	var $ssBaseOpOut;
	var $plugin_name = 'superslider-show';
	...
	function language_switcher(){
		...
		$superslider_locale = get_locale();
		$superslider_login_mofile = dirname(__FILE__) . "/languages/superslider-show-".$superslider_locale.".mo";
		$plugin_dir = dirname(plugin_basename(__FILE__));
		load_plugin_textdomain( $this->plugin_name, false, $plugin_dir.'/languages/' );
	...
	function ssShow_init() {
		...
		$this->js_path = WP_PLUGIN_URL.'/'. plugin_basename(dirname(__FILE__)) .'/js/';
  		$admin_js_path = WP_PLUGIN_URL.'/'.plugin_basename(dirname(__FILE__)) .'/admin/js/';
        ...
        wp_register_script( 'slimbox', $this->js_path.'slimbox.js', array( 'moomore' ), '2', false);        
       ...
       // wp_register_script( 'jquery-dimensions', $admin_js_path.'jquery.dimensions.min.js', array( 'jquery-ui-core' ), '2', false);
	    ...
	    wp_register_script( 'superslider-show-info', $admin_js_path.'superslider-show-info.js', array( 'jquery-ui-core' ), '2', false);
    	...
    	add_action ( "admin_init", array(&$this,"ss_print_box" ) ); // adds the shortcode meta box		
	...
	function ssShow_setup_optionspage() {
		if (  function_exists('add_options_page') ) {
			if (  current_user_can('manage_options') ) {
				$plugin_page = add_options_page(__('SuperSlider Show'),__('SuperSlider-Show'),'manage_options', 'superslider-show', array(&$this, 'ssShow_ui'));
	...
	function ssShow_create_css(){
        ...
        $this->css_path = WP_CONTENT_URL.'/'.$inner_path.'/'.$css_theme.'/'.$css_theme.'.css';
          ...
          $inner_path = 'plugin-data/superslider/ssShow';
         $this->css_path = get_stylesheet_directory_uri().'/plugin-data/superslider/ssShow/'.$css_theme.'/'.$css_theme.'.css';
         $this->light_path = get_stylesheet_directory_uri().'/'.$inner_path.'/'.$lightbox_type.'/'.$lightbox_type.'.css';  
        }
	...
	function ssShow_set_show($atts,$id) {
	   ...
	   extract($this->ssShowOpOut);
		...
		if ($css_load == 'default' || $css_load == 'off') {
			...
			$loaderPath = get_stylesheet_directory_uri().'/plugin-data/superslider/ssShow/loader/loader-#.png';
		}
		
		// give the show a name and number
		$showName = "this.el = $('slide_gallery".$this->show_id."');";
		
		if ( $accesskeys !== '' ) $myaccesskeys = "accesskeys: {".stripslashes($accesskeys)." },"; else $myaccesskeys = '';
		//if ( $properties !== '' ) $myproperties = "properties: [".stripslashes($properties)."],"; else $myproperties = '';

		if ( $replace !== '' && $fromfolder !== '') $myreplace = "replace:[/(\.[^\.]+)$/, '".$replace."$1'],"; else $myreplace = '';

		// Create the lightbox binders
		if (($linked == 'lightbox') && ($lightbox_add == 'on')) {
				if ($lightbox_type == 'SqueezeBox') {
					$mylightbox = 'SqueezeBox.initialize({
									...
									';						
				} elseif ($lightbox_type == 'Slimbox') {
				    add_action ( "wp_footer", array(&$this,"slimbox_linker"));
				    $show = 'slide_gallery'.$this->show_id;
				    $mylightbox = '';
				    
				} else {
					$mylightbox = 'var popbox = new '.$lightbox_type.'({ 
						   	onOpen : function(){ this.pause(true); }.bind(ssShow'.$this->show_id.'), 
						  	onClose : function(){ this.pause(false); }.bind(ssShow'.$this->show_id.'),
						  	initialWidth : 50,
							initialHeight : 50,
							showControls : true,
							showNumbers : true,
		...
		if ($mouseover == 'true') {
			$mymouse = '
			ssShow'.$this->show_id.'.el.retrieve(\'images\').addEvents({
		...
		// transfer options into js objects before constructing the slideshow js call.
		
		if ($thumbnails == 'true') {
			$mythumb = 'fast: '.$fast.',
			thumbnails: { duration: 700, transition: Fx.Transitions.Circ.easeOut},';
			
			/*$mythumbframe = "var frame = new Element('span', { 'class': 'thumbframe' });
                        $$('#ss-thumbnails ul li').each(function(el) {
                            frame.clone().inject(el);
                        });";*/
			
			$mythumbcover = "['a', 'b'].each(function(p){new Element('div', { 'class': 'overlay ' + p }).inject('ss-thumbnails'); });";
			} else{
			$mythumb = 'fast: false,
			thumbnails: false,';
			}
		$mycontroller = 'controller: false,';
		if ($controller == 'true') {
			$mycontroller = 'controller: {duration: 800, transition: Fx.Transitions.Sine.easeOut},';
			}
		if ($loader == 'true') {
			//$myloader = 'loader: { duration: 700, transition: Fx.Transitions.Sine.easeOut, \'animate\': [\''.$loaderPath.'\', 12] },';
			$myloader = 'loader:true,';
			} else { $myloader ='loader:false,'; }
		if ($captions == 'true') {
			$mycaptions = 'captions: {duration: 700, transition: Fx.Transitions.Sine.easeOut},';
			} else {$mycaptions = 'captions: false,';}
        ...
        } else {         /*'slideshow', 'first', 'prev', 'play', 'pause', 'next', 'last', 'images', 'captions', 'controller', 'thumbnails', 'hidden', 'visible', 'inactive', 'active', 'loader'*/
            $myclass = '';        
        }
				
		// if shortcode has set a new show type we need to load the required javascript	
		// this should be loaded into the footer,
		if ($this->shortcode_showtype == 'true') echo $mynewjs;
			
		// set a global href for the show if there is one
		if ( $href != '') { $myhref = 'href: "'.$href.'", linked: true,';}else{ $myhref = 'linked: false,';}
		...
		$initshow = "
			var ssShow".$this->show_id." = new ".$new_type."('slide_gallery".$this->show_id."', ".$data.",{
               center: ".$center.",
               delay: ".$delay.",
               duration: ".$duration.",
               hu: ".$hu.",
	...
	function slimbox_linker() {
			...
			$myslimbox ="<script type=\"text/javascript\">				
				...
				console.log('this is show');
				 var showimages = $(".$show.").getElements('a.slimbox');
				 console.log('this is show' +showimages);
				*/
			echo $myslimbox;
	...
	*/
	...
	return $i;*/
		...
		$atts = shortcode_atts(array(
			...
			'thumbnails' => '', 'thumbsize' => '', 'mouseover' => '', 'paused' => '', 
			'random' => '', 'loop' => '', 'loader' => '', 'delay' => '', 'duration' => '', 'controller' => '', 
			'exclude' => '', 'clear' =>	'', 'fromfolder' => '', 'css_theme' => '', 'ids' => '',
			'transition' => ''), $atts);
...
// This query is returning empty : it is used to get the latest posts , one from each category, I think. 
	$querystr = "SELECT post_date,ID, post_title, 
		...
		$last_posts = $wpdb->get_results($querystr,OBJECT);
      ...
      case ( strpos($id,'random@') ):            
       ...
       case ( strpos($id,'nextgen-') ):            
		...
		if ( empty($all_attachments) && ! empty($ids)) {
		 // there are no posts attached nore images attached, but we have a gallery listing via ids
			...
			foreach ( $_attachments as $key => $val ) {
				$all_attachments[$val->ID] = $_attachments[$key];
			}
				
		} 
		if ( empty($all_attachments) ) {
		 // there are no posts attached no images attached, no gallery listing via ids
			
		return 'There are no images here to be shown.';
	...
	// Open slideshow
		$output = apply_filters('show_style', "<div id=\"slide_gallery".$this->show_id."\" class=\"slideshow ".$show_class."\" style=\" height:".$height."px; width:".$width."px; \"><div class=\"slideshow-images show-$post->ID\" >");
     ...
     if ($captions == 'true') {  
        ...
        switch ($caption_text) {
         ...
         switch ($caption_link) {
            ...
            $insert_caption_link = '';
       if ( ($my_caption_link != '') && ($captions == 'true')) $insert_caption_link = " &lt;a href=&quot;".get_permalink($my_parent) ."&quot;&gt;".$my_caption_link."&lt;/a&gt; "; 
		...
		// output the main images		      		     
            $output .= '<img id="slide-'.$attachment->ID.'" src="'.$image[0].'" alt="'.$my_caption_title.' '.$my_caption_text.' '.$insert_caption_link.'" width="'.$image[1].'" height="'.$image[2].'" style="" />'; //style="visibility: hidden; opacity: 0;"
	...
	// Add controller
    if ( $controller == 'true')	{
        $output .= '<div class="slideshow-controller" style="opacity: 0;">
                 <ul>
	...
	*/
	...
	function ss_print_box($hook) {
		...
		//if(('post-new.php' == $hook) || ('post.php' == $hook)) {
			...
			if	($this->ssShowOpOut['show_shortcode'] == 'true')	{		
			...
			if ( ! current_user_can('edit_posts') && ! current_user_can('edit_pages'))
			   return;
				...
				if( function_exists( 'add_meta_box' )) {
					add_meta_box( 'ss_show', __( 'SuperSlider-Show', $plugin_name ), array(&$this,'ss_writebox'), 'post', 'advanced', 'high');
					add_meta_box( 'ss_show', __( 'SuperSlider-Show', $plugin_name ), array(&$this,'ss_writebox'), 'page', 'advanced', 'high' );
					
					add_action('admin_enqueue_scripts', array(&$this,'ss_enqueue_edit_js'));
